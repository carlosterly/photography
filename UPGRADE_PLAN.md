...
| **audit** | `npm audit fix` (no `--force`) — patched `immutable` 4.1.0→4.3.9, `shell-quote` 1.8.3→1.10.0, `picomatch` 2.3.1→2.3.2, `js-yaml` 3.14.1→3.15.2; all semver-compatible, `package.json` untouched. `npm audit` → 0 vulnerabilities; build output byte-identical. | `1ee92b8` |
| **vt-noise** | Added an `unhandledrejection` handler in `partials/body-close.njk` that `preventDefault()`s the benign `AbortError: transition was skipped` from cross-document view transitions. Renders on all 12 pages. | `1ee92b8` |
